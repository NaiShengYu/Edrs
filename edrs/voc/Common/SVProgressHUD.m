...
@property (nonatomic, readwrite) BOOL showGif;
...
@property (nonatomic, readonly) SvGifView *gifView;
...
@synthesize hudView, maskType, showNetworkIndicator, fadeOutTimer, stringLabel, imageView, spinnerView, previousKeyWindow, visibleKeyboardHeight,gifView;
...
+ (void)showGif{
    [[SVProgressHUD  sharedView] setShowGif:YES];
    [SVProgressHUD showWithStatus:nil networkIndicator:SVProgressHUDShowNetworkIndicator];
}
...
+ (void)showErrorWithStatus:(NSString *)string afterDelay:(NSTimeInterval)seconds{
    [SVProgressHUD show];
    [SVProgressHUD dismissWithError:string afterDelay:seconds];
}

+ (void)showSuccessWithStatus:(NSString *)string{
    [SVProgressHUD show];
    [SVProgressHUD dismissWithSuccess:string afterDelay:2];
}

+ (void)showErrorWithStatus:(NSString *)string{
    [SVProgressHUD show];
    [SVProgressHUD dismissWithError:string afterDelay:2];
}
...
- (void)setStatus:(NSString *)string {
	...
	if(string)
        if(_showGif){
           self.gifView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, 40.5);
        }else{
          self.spinnerView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, 40.5);
        }
	else
        if(_showGif){
            	self.gifView.center = CGPointMake(ceil(CGRectGetWidth(self.hudView.bounds)/2)+0.5, ceil(self.hudView.bounds.size.height/2)+0.5);
        }else{
...
- (void)positionHUD:(NSNotification*)notification {
    ...
    CGFloat activeHeight = 0;
//    if(iPhone){
        activeHeight  = orientationFrame.size.height;
//    }else{
//        activeHeight =  orientationFrame.size.width;
//    }
...
//    if(iPhone){
        ...
        posX = orientationFrame.size.width/2;
//    }else{
//        posY = floor(activeHeight*0.45);
//        posX = orientationFrame.size.height/2;
//    }
    CGPoint newCenter;
    CGFloat rotateAngle;
...
- (UIView *)hudView {
    ...
    if(!hudView) {
//        BOOL iPhone = [deviceModel hasPrefix:@"iPhone"];
...
- (SvGifView *)gifView{
    if(gifView ==nil){
        ...
        gifView = [[SvGifView alloc]initWithFrame:CGRectZero fileURL:fileUrl];
        gifView.bounds = CGRectMake(0, 0, 64, 64);
        [self.hudView addSubview:gifView];
    }
    return gifView;
